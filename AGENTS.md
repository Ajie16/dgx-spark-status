...
11. **Multi-node supervision**: every node runs its own `dev-server`. A primary sets `PEER_NODES` (`name=url` pairs or a JSON array, e.g. `spark-2=http://<peer-ip>:9000`) and polls each peer's `GET /api/snapshot` on every tick (2.5s timeout). The merged `metrics.nodes` array (local + peers with `online/error/lastSeen` and last-known metrics) is embedded in the SSE payload; `/api/snapshot` strips the `nodes` field so peer-of-peer data cannot nest. The UI renders a node strip in the header area when more than one node is present; **clicking a chip switches the entire dashboard inline to that node's data** (same-origin, no navigation — third devices never need to route to peer IPs). Peer charts seed from `GET /api/history?node=<name>` (primary proxies the peer's history server-side). Local-only controls (fan modes, ComfyUI start/stop, notes) are hidden in remote views. A reverse proxy per peer (`startPeerProxy`, ports 9101, 9102, …) remains available for direct standalone access.
...
| `GET /api/snapshot` | ✅ | ❌ | Latest metrics as plain JSON (no SSE), used for peer aggregation; CORS `*` |
